...
async function main() {
  ...
  const RaptorNFTFactory = await hre.ethers.getContractFactory(
    "RaptorNFT"
  );
  const raptorNFT = await RaptorNFTFactory.attach(CONTRACT_ADDRESS);
  ...
  for (let i = 0; i < 5; i++) {
    const depositTx = await fxRootContract
      .connect(deployer)
      .deposit(raptorNFT.address, BRIDGE_ADDRESS, i, "0x6566");
